...
class Master extends EventEmitter {
  constructor (targets, configFile, concurrency) {
    ...
    this._configPath = path.join(process.cwd(), configFile)
  ...
  _forkWorkers (count) {
    ...
    for (let i = 0; i < count; ++i) {
      cluster.fork({
        FANCY_ROLLUP_CONFIG_PATH: this._configPath,
...
const getOpts = availableReporters => {
  const { target: targets, config: configFile, concurrency, reporter: reporters } = yargs
    .alias('v', 'version')
    .alias('h', 'help')
    .option('t', {
      alias: 'target',
    ...
    .parse()
  return { targets, configFile, concurrency, reporters }
}

const runMaster = async () => {
  const availableReporters = enumerateReporters()
  const { targets, configFile, concurrency, reporters } = getOpts(availableReporters)
  const master = new Master(targets, configFile, concurrency)
  handleFatalErrors(error => {
    master.emit('fatal', { error })
  })
  for (const name of reporters) {
    master._loadReporter(name)
  }
  await master._run()
}
