...
const titleRenderers = {
  ...
  [STATE_RUNNING]: ({ target, startTime }) => target + ' ' + cyan(formatDuration(Date.now() - startTime)),
  [STATE_SUCCEEDED]: ({ target, totalTime }) => green(target) + ' ' + cyan(formatDuration(totalTime, true)),
  [STATE_FAILED]: ({ target, totalTime }) => red(target) + ' ' + cyan(formatDuration(totalTime, true))
}

const updateTitle = meta => {
  meta.task.title = titleRenderers[meta.state](meta)
}
